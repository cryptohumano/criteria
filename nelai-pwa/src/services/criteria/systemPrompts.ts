...
[Recordatorio interno CriterIA: en análisis de documento o PDF, coloca PRIMERO el bloque técnico [SCORE_LEGAL]…[/SCORE_LEGAL] (puntuación 1–100, nivel ALTO|MEDIO|BAJO, resumen, riesgos o aspectos a mejorar separados por |). En modo académico ese bloque representa calidad académica; en modo legal, confianza legal. DESPUÉS el análisis detallado. No omitas el bloque.]`
...
export type AgentProfile = 'legal_mx' | 'academic_es'
...
export function agentSystemPromptForProfile(profile: AgentProfile): string {
  return profile === 'legal_mx' ? DOCUMENT_AGENT_SYSTEM_PROMPT : ACADEMIC_AGENT_SYSTEM_PROMPT
}
...
export function withAgentProfileTag(
  ...
  const tag = profile === 'legal_mx' ? AGENT_PROFILE_TAG_LEGAL_MX : AGENT_PROFILE_TAG_ACADEMIC_ES
  return [...without, tag]
}
