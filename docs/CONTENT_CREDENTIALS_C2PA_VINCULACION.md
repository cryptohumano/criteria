# Content Credentials (C2PA) y CriterIA: estándar base + Polkadot
...
- **Redundancia y resiliencia:** El mismo activo lleva un **manifiesto C2PA conforme** (claim + assertions + firma según spec) y, opcionalmente, una **assertion personalizada** con identidad/firma Polkadot. Así:
  ...
  - CriterIA (y forks) pueden verificar además la capa Polkadot (soberanía, on-chain si se desea).
...
| Aspecto | Enfoque CriterIA (alineado a C2PA) |
|--------|----------------------------------|
...
| **Identidad Polkadot** | Assertion personalizada (p. ej. `org.nelai.polkadot`): `address` (ss58), `signature` sobre el mismo payload que el claim, opcionalmente `signedPayload` o referencia. Así cualquier validador C2PA ve un credential válido; CriterIA y forks verifican además la firma Polkadot. |
...
- **Generación de manifiestos C2PA:**  
  ...
  - Firmar el claim (COSE): con X.509 para compatibilidad total o, en fase inicial, documentar que la verificación "completa" la hace CriterIA usando la assertion Polkadot.
...
- **Identidad C2PA (signer del claim):**  
  ...
  - Opción B: en MVP, firma del claim con herramienta que acepte claves no-X.509 si el SDK lo permite; la assertion Polkadot sigue siendo la prueba de identidad soberana para CriterIA/forks.
...
| **Después** | Certificado X.509 para el signer del claim C2PA (para que validadores estándar den "trusted" sin depender de CriterIA). Mantener la assertion Polkadot para redundancia y soberanía. |
